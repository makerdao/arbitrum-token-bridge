...
contract L1TokenGatewayTest is DssTest {
    ...
    function testTokenRegistration() public {
        assertEq(gateway.l1ToL2Token(address(111)), address(0));
        assertEq(gateway.calculateL2TokenAddress(address(111)), address(0));

        vm.expectEmit(true, true, true, true);
        emit TokenSet(address(111), address(222));
        gateway.registerToken(address(111), address(222));

        assertEq(gateway.l1ToL2Token(address(111)), address(222));
        assertEq(gateway.calculateL2TokenAddress(address(111)), address(222));
    }
